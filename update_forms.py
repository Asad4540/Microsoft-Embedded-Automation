...
def generate_folder_name_from_url(url):
    ...
    if path_parts:
        return path_parts[
            0
        ]  # Return the first part after the domain as the folder name
    else:
        return "default_folder"  # Fallback if no valid folder name can be extracted


# Function to generate a unique filename based on the last three parts of the URL path
def generate_filename_from_url(url, existing_filenames):
    ...
    base_filename = "-".join(
        last_three
    )  # Join the parts with "-" to form the base filename
    ...
    existing_filenames.add(
        filename
    )  # Add the generated filename to the set of existing filenames
    return filename
...
def process_excel(file_path):
    ...
    existing_filenames = set()  # Set to keep track of filenames that have been used
    ...
    for index, row in df.iterrows():  # Loop through each row in the Excel file
        ...
        images_folder = os.path.join(
            folder_name, "images"
        )  # Images folder inside the new folder
        os.makedirs(
            images_folder, exist_ok=True
        )  # Create the images folder if it doesn't exist

        modified_html = replace_form_content(
            html_content, new_form_snippet, redirect_url, images_folder, url
        )  # Replace the form content and update the HTML

        if modified_html:
            file_name = generate_filename_from_url(
                url, existing_filenames
            )  # Generate a unique filename for the modified HTML
            save_modified_html(
                folder_name, file_name, modified_html
            )  # Save the modified HTML to a file inside the new folder
            print(
                f"Successfully modified and saved HTML for {url} as {file_name} in folder {folder_name}"
            )
        else:
...
if __name__ == "__main__":
    excel_file_path = "webpages_and_snippets.xlsx"  # Path to the Excel file containing URLs and snippets
